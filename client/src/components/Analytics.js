...
function Analytics({ transactions, type }) {
  ...
  const EmptyPlaceholder = ({ message }) => (
    <div className="empty-placeholder">{message}</div>
  );
  ...
  const CategoryWiseTransactionChart = ({ transactionType, totalTurnOver }) => {
    const categoryList = categories
      .filter((category) =>
        transactions.some(
          (transaction) =>
            transaction.type === transactionType && transaction.category === category
        )
      )
      .map((category) => {
    ...
    return categoryList.length ? (
      ...
      <EmptyPlaceholder message={`No ${transactionType} found for the selected duration and type`} />
    );
  };
          ...
          <div className="transactions-count">
            ...
            <div className="progress-bars">
              <Progress
                ...
                strokeColor="#5DD64F"
                type="circle"
                percent={((totalIncomeTurnover / totalTurnover) * 100).toFixed(0)}
              />
              <Progress
                strokeColor="#E5572F"
                type="circle"
                percent={((totalExpenseTurnover / totalTurnover) * 100).toFixed(0)}
              />
            </div>
          </div>
        </div>
